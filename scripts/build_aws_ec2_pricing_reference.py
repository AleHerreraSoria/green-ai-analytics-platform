...
def _load_aws_regions(root: Path) -> list[str]:
    path = root / "bronze/reference/geo_cloud_to_country_and_zones.csv"
    with path.open(encoding="utf-8", newline="") as f:
        r = csv.DictReader(f)
        return [row["cloud_region"] for row in r if row.get("cloud_provider") == "aws"]


def _load_aws_instance_types(root: Path) -> list[str]:
    path = root / "data/Code_Carbon/instances.csv"
    out: list[str] = []
    with path.open(encoding="utf-8", newline="") as f:
        r = csv.DictReader(f)
        for row in r:
...
def main() -> int:
    root = _repo_root()
    regions = _load_aws_regions(root)
    instances = _load_aws_instance_types(root)
    ...
    out_path = root / "bronze/reference/aws_ec2_on_demand_usd_per_hour.csv"
